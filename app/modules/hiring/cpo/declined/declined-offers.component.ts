...
export class DeclinedOffers {
  ...
  constructor(
    ...
    this.hiringComponent.URLtitle = "Offers / Declined Offers";
    this.columnDefs = [
  ...
  loadGrid() {
    ...
    setTimeout(() => {
      ...
      this.offersDataService.getOffersByStatus(this.pageNum, this.pageSize, 'DECLINED').subscribe(data => {
  ...
  exportData() {
    var self = this;
    setTimeout(() => {
      self.offersDataService.exportOffers('DECLINED').subscribe(data => {
        self.commonService.downloadCsv('Declined Offers', data['_body']);
      }, error => {
        self.errorHandleService.handleErrors(error);
      });
    }, 100);
