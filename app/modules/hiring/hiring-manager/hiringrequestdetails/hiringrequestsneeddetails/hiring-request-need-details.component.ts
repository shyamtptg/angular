...
@Component({
  selector: 'hiring-request-need-details',
  templateUrl: './hiring-request-need-details.component.html'
})

export class RecruiterHiringNeedDetails {
  @Input() needData: any;
  @Input() needIndex: any;
  @Input() comboData: any;
  ...
  toggleHiringNeedDetails() {
    ...
    if (hiringNeedDetails.eq(this.needIndex).hasClass('close')) {
      ...
      setTimeout(() => {
        var body = $("html");
        body.stop().animate({ scrollTop: $('hiring-request-need-details').eq(self.needIndex).offset().top + 56 * (self.needIndex) - 75 }, 500, 'swing');
      }, 650);
    } else {
      hiringNeedDetails.eq(this.needIndex).slideToggle(700);
      setTimeout(() => {
