...
@Component({
    templateUrl: './prejoining-pending.component.html'
})

export class PrejoiningPending {
    columnDefs: any;
    ...
    constructor(
        ...
        this.hiringComponent.URLtitle = "Profiles / Pre-Offer Pending";
        ...
        this.columnDefs = [
            ...
            { headerName: "Current Employer", field: "currentEmployeer", minWidth: 130, tooltipField: "clientName", currentEmployeer: "Current Employer" },
        ...
        this.status = "PENDING_PREJOINING_FORMALITIES";
    }
    createNewDatasource() {
    ...
    exportData() {
        ...
        if (this.hiringRequestId && !this.hiringRequestNeedId) {
            this.addProfileDataService.exportAllProfilesByHiringIdAndFilters(this.hiringRequestId, this.noticePeriod, this.status).subscribe(data => {
                data = data['_body'];
                self.commonService.downloadCsv('Prejoining Pending', data);
            }, error => {
                self.errorHandleService.handleErrors(error);
            });
        } else if (this.hiringRequestId && this.hiringRequestNeedId) {
