...
@Component({
    templateUrl: './completed-prejoining.component.html'
})

export class CompletedPrejoining {
    columnDefs: any;
    ...
    constructor(
        ...
        this.hiringComponent.URLtitle = "Profiles / Ready To Verify";
        ...
        this.status = "VERIFICATION_IN_PROGRESS";
    }
    ...
    exportData() {
        ...
        if (this.hiringRequestId && !this.hiringRequestNeedId) {
            this.addProfileDataService.exportAllProfilesByHiringIdAndFilters(this.hiringRequestId, this.noticePeriod, this.status).subscribe(data => {
                data = data['_body'];
                self.commonService.downloadCsv('Ready To Verify', data);
            }, error => {
                self.errorHandleService.handleErrors(error);
            });
        } else if (this.hiringRequestId && this.hiringRequestNeedId) {
