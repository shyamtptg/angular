...
export class RecruiterSelected {
    columnDefs: any;
    ...
    constructor(
        ...
        this.hiringComponent.URLtitle = "Profiles / Selected Profiles";
    ...
    notifyCandidate() {
        ...
        if (selectedRecords.length > 0) {
            ...
            this.profilesDataService.notifyCandidate(prospectId, payload).subscribe(data => {
                if (data.status == 200) {
    ...
    exportData() {
        ...
        if (this.hiringRequestId && !this.hiringRequestNeedId) {
            this.addProfileDataService.exportAllProfilesByHiringIdAndFilters(this.hiringRequestId, this.noticePeriod, this.status).subscribe(data => {
                data = data['_body'];
                self.commonService.downloadCsv('Selected Profiles', data);
            }, error => {
                self.errorHandleService.handleErrors(error);
            });
        } else if (this.hiringRequestId && this.hiringRequestNeedId) {
    ...
    checkModes() {
        ...
        if (selectedRecords.length > 0) {
            ...
            this.buttonModes['notifyCandidate'] = false;
        } else {
            this.resetModes();
        }
    }
